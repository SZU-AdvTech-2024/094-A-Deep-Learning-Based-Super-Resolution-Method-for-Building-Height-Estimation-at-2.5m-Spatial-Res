...
class myImageFloder_S12_globe(data.Dataset):
    def __init__(self, datalist, rootname, datastats='datastats',
        ...
        self.rootname = rootname
        print('datastats',datastats)
        #print('数据',np.loadtxt('D:\Desktop\Super-resolution-building-height-estimation-main\datasetglobe\s1globe_check_minmax.txt'))
        self.norms1 = np.loadtxt(os.path.join(datastats,f'{s1dir}_{normmethod}.txt'))
        print('norms1',self.norms1)
    ...
    def __getitem__(self, index):
        ...
        # s2 & s1 images   拼接S1和S2图像
        s2 = tif.imread(os.path.join(self.rootname, s2dir, basename))[:, :, :self.nchans]# 维度为 H,W,nchans
