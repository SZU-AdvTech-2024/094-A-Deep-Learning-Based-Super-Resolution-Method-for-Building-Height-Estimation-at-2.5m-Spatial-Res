...
os.environ['TORCH_HUB_CHECHPOINT_DIR']='/home2/yzt/efficientnet-b4-6ed6700e.pth'
import torch
import random
import numpy as np
...
def get_args(city='globe'):
    ...
    parser.add_argument('--logdirhr', default=fr'./weights/realesrgan/checkpoint.tar') # add
    ...
    parser.add_argument('--datastats', type=str, default='datasetglobe')
    #parser.add_argument('--datastats', type=str, default='datastatsglobe')
    parser.add_argument('--preweight', type=str, default=f'datasetglobe/bh_stats_{city}.txt', help='None') # weight
...
def main_test(args, num_sample=100, suffix='',
    ...
    # Setup datalist
    data_path = args.datapath#./data
    testlist_path = os.path.join(data_path, args.testlist)# datalist_{city}_test_0.7_test_0.3.csv
    logdir_hr = args.logdirhr#./weights/realesrgan/checkpoint.tar

    # Setup parameters
    classes = args.chans_build #('--chans_build', type=int, default=7)
    nchannels = args.nchans  # ('--nchans', default=8)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    logdir = args.logdir  #./weights/realesrgan_feature_aggre_weight_{city}

    # num_sample = 100
    '''
    torch.utils.data.DataLoader 是 PyTorch 中的数据加载工具，用于高效地从数据集中加载数据到模型中进行训练和测试。它可以支持批量加载、数据打乱、多线程加速等功能
    batch_size:每个批次的加载量
    num_workers:加载数据的子进程数量
    pin_memory:如果为True，DataLoader将数据加载到固定内存中，加速GPU数据传输
    ...
    # super-resolution semantic segmentation   超分辨率语义分割
    # from mymodels
    # return height, build
    net = SRRegress_Cls_feature(encoder_name="efficientnet-b4",
                                   in_channels=nchannels, super_in=64,
    ...
    # Super-resolution image reconstruction    超分辨率图像重建
    # from SR
    # feat
    net_hr = RealESRGAN(pretrain_g_path=None,
                     pretrain_d_path=None,
    ...
    #net_hr.net_g.load_state_dict(torch.load(logdir_hr)['net_g_ema'])
    net_hr.net_g.load_state_dict( torch.load(logdir_hr, map_location='cuda', weights_only=True)['net_g_ema'])
    net_hr.net_g.eval()
    for p in net_hr.net_g.parameters():
        p.requires_grad = False
...
if __name__=="__main__":
    ...
    print("-------------------------------")
    main(args)
    args.checkpoint = 'checkpoint2.tar'
