...
def get_args(city='globe'):
    ...
    parser.add_argument('--cityname', type=list, default=["Anqing"]) # ningbo, tianjin, lanzhou
    # parser.add_argument('--grid', type=int, default=64)
    # parser.add_argument('--stride', type=int, default=60) # overlap 4 pixels =40m = 2.5m*16
    args = parser.parse_args()
    return args
'''
suffix=city+iosname('chn_large', 'usa_large', 'europe_large',  'chn_metro', 'usa_metro', 'europe_metro')
...
def main_test(args, num_sample=100, suffix='',
    ...
    testlist_path = os.path.join(data_path, args.testlist)#./data/datalist_globe_test_0.7_test_0.3.csv
    logdir_hr = args.logdirhr#./weights/realesrgan/checkpoint.tar

    # Setup parameters
    classes = args.chans_build
    nchannels = args.nchans#8
    device = 'cuda'
    ...
    net_hr = RealESRGAN(pretrain_g_path=None,
                     ...
                     num_block=23)
    net_hr.net_g.load_state_dict(torch.load(logdir_hr)['net_g_ema'])#将字典中net_g_ema的部分加载到net_hr.net_g中
    net_hr.net_g.eval()#将模型转为评估模式
    ...
    if iswhole:
        ...
        for cityname in args.cityname:#cityname=lanzhou
...
def predict_whole_image_grid(args, cityname, model, net_hr, device,
    ...
    print("args.wholeimgpath   ",args.wholeimgpath)
    print("cityname",cityname)
    print("datastats",args.datastats)
    # load img
    ...
    with torch.no_grad():
        ...
        # building prediction
        print("1231q31e3qe3wq2e1         ", res_build.shape)
        ...
        print("res_height",res_height.shape)
