...
'''
RDB模块通过深度残差连接和密集连接来增强信息流动和梯度传播

Residual Dense Block (RDB) 是一个残差密集块，在 RRDB（Residual in Residual Dense Block）中使用，这通常是 ESRGAN 中的一部分。
num_feat 参数：表示特征图的通道数（每个块的输入和输出通道数）。
num_grow_ch 参数：表示每次卷积操作后通道数的增长量。
'''
...
num_in_ch: 输入图像的通道数（例如，RGB 图像为 3）。
num_out_ch: 输出图像的通道数，通常用于图像超分辨率时，输出通道数与输入相同，或者有些任务需要不同的通道数。
scale: 上采样的倍数，默认值为 4，表示最终输出的图像将比输入图像的尺寸大 4 倍。
num_feat: 特征图的通道数，用于表示网络的容量（默认为 64）。
num_block: 网络中 RRDB 模块的数量（默认为 23）。
num_grow_ch: 每个 RRDB 模块中增长的通道数（默认为 32）。
...
class RRDBNet(nn.Module):
    def __init__(self, num_in_ch, num_out_ch, scale=4, num_feat=64, num_block=23, num_grow_ch=32):
        ...
        self.conv_first = nn.Conv2d(num_in_ch, num_feat, 3, 1, 1)#定义了一个卷积层 num_in_ch:输入通道数  num_feat:输出通道数  3:卷积核大小  1：步长 1：填充
        self.body = make_layer(RRDB, num_block, num_feat=num_feat, num_grow_ch=num_grow_ch)#23个RRDB
        self.conv_body = nn.Conv2d(num_feat, num_feat, 3, 1, 1)#Conv2
        # upsample
...
out_ch:输出通道数
scale:超分辨率的倍数
ema_decay:指数移动平均的衰减因子，用于稳定推理时的生成器参数,
较大的 ema_decay（如 0.99 或更大）：表示模型参数的更新较为平滑，历史更新的权重较大。这样可以让模型的权重变得更加稳定，从而提升模型的泛化能力，尤其是在训练数据较为复杂时。
pretrain_g_path, pretrain_d_path：用于加载预训练的生成器和判别器权重的路径。
is_train：是否用于训练，若为 False，则为推理模式。
'''
class RealESRGAN:
    def __init__(self, in_ch=3, out_ch=3, num_block=23, device='cuda', scale=4, ema_decay=0.999,
        ...
        self.device = device
        self.scale = scale
        self.usm_sharpener = USMSharp().cuda()#将USMSharp转到CUDA上计算
