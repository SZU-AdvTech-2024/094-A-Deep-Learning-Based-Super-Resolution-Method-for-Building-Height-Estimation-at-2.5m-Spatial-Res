...
# use residual block  低分辨率和高分辨率图像进行融合
...
if __name__ == '__main__':
    ...
    # print(model) # 0.01 M
    # 使用 rasterio 加载多波段的 tif 文件
    with rasterio.open('../data/s2china_check/baoding_4.tif') as src:
        img = src.read()  # 读取所有波段，结果是一个 numpy 数组 (C, H, W)

    # 转换为 PyTorch Tensor
    img_tensor = torch.from_numpy(img).float()  # 转换为 float 类型的 Tensor

    # 定义预处理变换
    transforms = transforms.Compose([
        transforms.Normalize(mean=[0.5] * img.shape[0], std=[0.5] * img.shape[0])  # 针对每个波段设置均值和标准差
    ])
    ...
    # 检查张量的大小
    print("图像张量的形状:", img_tensor.size())  # 形状应为 (C, H, W)


    # 创建一个 Upsampler 实例
    upsampler = Upsampler(conv=default_conv, scale=4, n_feats=16)

    # 通过模型进行上采样
    output = upsampler(img_tensor)
