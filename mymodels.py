...
#import segmentation_models_pytorch.decoders.unet
...
class SRRegress_Cls_feature(torch.nn.Module):
    '''
    encoder_name 和 encoder_weights: 控制编码器的种类（如 ResNet）和预训练权重来源（如 ImageNet）。
    encoder_depth: 编码器的深度，影响特征提取的分辨率。
    in_channels: 输入通道数，比如光学图像和 SAR 图像的通道数。
    super_in 和 super_mid: 控制超分辨率分支的输入通道数和中间特征维度。
    upscale: 上采样倍数。
    isaggre: 是否使用聚合模块。
    chans_build: 用于建筑分类输出的通道数。
    uniform_range: 设置权重初始化范围。
    isunsup: 是否使用无监督模式。
    '''
    def __init__(self, encoder_name="resnet50", encoder_weights="imagenet", encoder_depth=5,
                 in_channels=7, classes=1, super_in=4, super_mid=64, upscale=4,
        ...
        # Super-Resolution branches
        # self.super_res = EDSR_feaHR(n_colors=super_in, n_out=3, n_feats=super_mid)
        # Encoder-Decoder    efficientnet-b4
        self.encoder = get_encoder(encoder_name, in_channels=in_channels,
                                   depth=encoder_depth,)
        #self.encoder.load_state_dict(torch.load(local_weights_path, map_location=torch.device('cpu')))
